...
export default function SplitText({ children }: SplitTextProps) {
  ...
  useEffect(() => {
    ...
    nodes.forEach(node => {
      if (node.nodeType === Node.TEXT_NODE && node.textContent?.trim()) {
        ...
        // mantém espaços usando regex
        const words = node.textContent.split(/(\s+)/)

        words.forEach(word => {
          if (/\s+/.test(word)) {
            // espaço em branco → apenas adiciona como textNode
            fragment.appendChild(document.createTextNode(word))
          } else {
            const span = document.createElement('span')
            span.textContent = word
            span.dataset.word = word
            // apenas necessário o inline-block para a animação y
  ...
  return <span ref={containerRef} className={styles.split}>{children}</span>
}
